    ...
    .then((resp) => {
      ...
      if (resp.status == true) {
        ...
        if (resp.data.length != 0) {
          ...
          {
            if (element.answerContent.length != 0) {

              card += `<div class="content">${element.description}</div>`;
                card += ` <button class="accuracy" id="seen" onclick=getAnswers(${element.questionId}) >Answers</button>`;
